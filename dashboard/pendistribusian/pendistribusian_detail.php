<?php
    ...
    if(!isset($_SESSION['username'])){
        ...
        $sql_zakat = "SELECT SUM(pendistribusian) AS total_zakat FROM view_813a";
        $hasil_zakat = $con->query($sql_zakat);
        $row_zakat = $hasil_zakat->fetch_assoc();
        $total_zakat = number_format($row_zakat['total_zakat'],0,',','.');

        $sql_infak = "SELECT SUM(pendistribusian) AS total_infak FROM view_813b";
...
<div class="row">
    <div class="col-lg-12">
        <h3>Detail Pendistribusian ZISKA</h3>
        <ol class="breadcrumb">
            <li><a href=""><i class="glyphicon glyphicon-signal"></i></a></li>
            <li class="active">Grafik Detail Pendistribusian</li>
...
<script type="text/javascript">
    $(function(){
        ...
        $(document).ready(function() {
            $.getJSON("pendistribusian/data_pendistribusian_detail.php", function(json) {
                grafik = new Highcharts.Chart({
                    ...
                    title: {
                        text: 'Detail Pendistribusian ZISKA Berdasar Periode Bulanan'
